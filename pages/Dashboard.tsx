...
const Dashboard: React.FC<DashboardProps> = ({ setActivePage, attendanceRecords, leaveRequests }) => {
  ...
  const attendanceChartData = useMemo(() => {
    ...
    return weekDays.map((dayName, index) => {
      ...
      const dailyRecords = attendanceRecords.filter(r => r.date === dateStr);
      const hadir = dailyRecords.filter(r => r.status === 'Hadir').length;
      // Count all non-present statuses as "Tidak Hadir" (represented as Alpha in chart data key)
      const alpha = dailyRecords.filter(r => r.status !== 'Hadir').length;
