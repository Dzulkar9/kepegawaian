...
const DasborPegawai: React.FC<DasborPegawaiProps> = ({
  ...
  const [successModal, setSuccessModal] = useState<{ isOpen: boolean; title: string; message: string; subMessage?: string; time?: string }>({
    isOpen: false,
    title: '',
    message: ''
  });
  ...
  const executeAttendanceAction = () => {
    if (attendanceAction === 'in') {
      ...
      setSuccessModal({
        ...
        subMessage: 'Selamat bekerja dan semangat!',
        time: timeString
      });
      ...
      setSuccessModal({
        ...
        message: 'Absensi pulang Anda telah tercatat.',
        subMessage: 'Terima kasih atas kerja keras Anda hari ini.',
        time: timeString
      });
    }
